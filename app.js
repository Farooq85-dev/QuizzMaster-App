function paragraph_show1() {
  ...
  var nonExpanded_Para1 =
    "The introduction to HTML, or Hypertext Markup Language, serves as the foundational gateway into the realm of web development. HTML is the bedrock of web pages, providing a standardized markup system that structures content on the internet.";
...
function paragraph_show3() {
  ...
  var nonExpanded_Para3 =
    "JavaScript, often abbreviated as JS, stands as the dynamic force behind interactivity and client-side functionality on the web.";
...
var htmlQuestions = [
  ...
  {
    Question:
      "Which tag is used in HTMLFive for the initialization of the document type?",
...
function startHtmlQuizz() {
  ...
  for (var i = 0; i < options.length; i++) {
    if (options[i].checked) {
      ...
      break; // Break after finding a selected option
    }
  }

  // If no option is selected, show an alert and do not proceed
  if (indexhtml > 0 && !isOptionSelected) {
...
function startCssQuizz() {
  ...
  // If no option is selected, show an alert and do not proceed
  if (indexcss > 0 && !isOptionSelected) {
    alert("Please select an option before proceeding.");
    return;
  }
